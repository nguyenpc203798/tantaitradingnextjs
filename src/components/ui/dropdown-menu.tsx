...
// Original dropdown menu (focus-based)
const DropdownMenu = DropdownMenuPrimitive.Root

// Hover dropdown menu
const HoverDropdownMenu: React.FC<
  React.ComponentPropsWithoutRef<typeof DropdownMenuPrimitive.Root> & {
    openDelay?: number;
    closeDelay?: number;
  }
> = ({ children, openDelay = 100, closeDelay = 150, ...props }) => {
...
>(({ className, sideOffset = 4, preventScroll = false, ...props }, ref) => {
  // Avoid DOM manipulation for non-SSR env
  React.useEffect(() => {
    // Backup original overflow
    const originalOverflow = document.body.style.overflow;
    
    // Don't block scroll unless explicitly wanted
    if (!preventScroll) {
      document.body.style.overflow = '';
    }
    
    return () => {
      // Restore to original state on cleanup
